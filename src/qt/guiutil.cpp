...
{
    if(url.scheme() != QString("winnercoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::WNR, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert winnercoin:// to winnercoin:
    //
    //    Cannot handle this later, because winnercoin:// will cause Qt to see the part after // as host,
    //    which will lowercase it (and thus invalidate the address).
    if(url.startsWith("winnercoin://"))
    {
        url.replace(0, 10, "winnercoin:");
    }
