...
{
    return QCoreApplication::translate("winnercoin-core", psz).toStdString();
}
...
int main(int argc, char *argv[])
{
    ...
    {
        if (strlen(argv[i]) > 7 && strncasecmp(argv[i], "winnercoin:", 8) == 0)
        {
    ...
    Q_INIT_RESOURCE(winnercoin);
    QApplication app(argc, argv);

    // Command-line options take precedence:
    ParseParameters(argc, argv);

    // ... then winnercoin.conf:
    ...
    app.setOrganizationName("Winnercoin");
    app.setOrganizationDomain("winnercoin.org");
    if(GetBoolArg("-testnet")) // Separate UI settings for testnet
        app.setApplicationName("Winnercoin-Qt-testnet");
    else
        app.setApplicationName("Winnercoin-Qt");
