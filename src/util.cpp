...
#else
    const char* pszModule = "winnercoin";
#endif
    if (pex)
...
void ThreadOneMessageBox(string strMessage)
{
    ...
    ThreadSafeMessageBox(strMessage, "Winnercoin", wxOK | wxICON_EXCLAMATION);
...
{
    // Windows: C:\Documents and Settings\username\Application Data\Winnercoin
    // Mac: ~/Library/Application Support/Winnercoin
    // Unix: ~/.winnercoin
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) + "\\Winnercoin";
#else
...
string GetConfigFile()
{
    ...
    fs::path pathConfig(GetArg("-conf", "winnercoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return true; // No winnercoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override winnercoin.conf
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong Winnercoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    boost::thread(boost::bind(ThreadSafeMessageBox, strMessage+" ", string("Winnercoin"), wxOK | wxICON_EXCLAMATION, (wxWindow*)NULL, -1, -1));
                }
            }
        }
        if (fDebug) {
...
// Format the subversion field according to BIP 14 spec (https://en.winnercoin.it/wiki/BIP_0014)
